...
class UploadFormHelper extends AppHelper {
	...
	*/
	public function load( $project_id )
	{
		...
		$this->_loadTemplate( $url, $project_id );
		...
		{% for (var i=0, file; file=o.files[i]; i++) { %}
		    <tr class="template-download fade">
		        {% if (file.error) { %}
		            ...
		            <td class="name"><span>{%=file.name%}</span></td>
		            <td class="size"><span>{%=o.formatFileSize(file.size)%}</span></td>
		            <td class="error" colspan="2"><span class="label label-important">{%=locale.fileupload.error%}</span> {%=file.error%}</td>
		        {% } else { %}
		        ...
		        <td class="delete">
		            <button class="btn btn-small btn-small btn-danger" data-type="{%=file.delete_type%}" data-url="{%=file.delete_url%}">
	...
	* 	@return void
	*/
	private function _loadTemplate( $url = null, $project_id = null )
	{
		echo '
		<form id="fileupload" action="'.Router::url('/', true).$url.'?project_id='.$project_id.'" method="POST" enctype="multipart/form-data">
	        <div class="row-fluid fileupload-buttonbar">
	            <div class="span7">
	                ...
	                <button type="reset" class="btn btn-small btn-warning cancel hide">
	                    <i class="icon-ban-circle icon-white"></i>
	        ...
	        <br>
	        <table class="table table-striped table-highlight table-condensed">
	        	<thead>
	        		<th>&nbsp;</th>
	        		<th>Name</th>
	        		<th>Size</th>
		...
		<div id="modal-gallery" class="modal modal-gallery hide fade" data-filter=":odd">
		    ...
		    <div class="modal-footer">
		        <a class="btn btn-small modal-download" target="_blank">
		            <i class="icon-download"></i>
		            <span>Download</span>
		        </a>
		        <a class="btn btn-small btn-success modal-play modal-slideshow" data-slideshow="5000">
		            <i class="icon-play icon-white"></i>
		            <span>Slideshow</span>
		        </a>
		        <a class="btn btn-small btn-info modal-prev">
		            <i class="icon-arrow-left icon-white"></i>
		            <span>Previous</span>
		        </a>
		        <a class="btn btn-small btn-primary modal-next">
		            <span>Next</span>
	...
	*/
	private function _loadExternalJsFiles()
	{
		echo '
			...
			<script src="http://blueimp.github.com/JavaScript-Templates/js/tmpl.min.js"></script>
			<script src="http://blueimp.github.com/JavaScript-Load-Image/js/load-image.min.js"></script>
			<script src="http://blueimp.github.com/JavaScript-Canvas-to-Blob/js/canvas-to-blob.min.js"></script>
